<template>
  ...
  >
    <template v-slot:activator="{ on, attrs }">
      <v-btn color="primary" text small v-bind="attrs" @click="open">
        Xem
      </v-btn>
    </template>
    <v-toolbar
    ...
    >
      <v-toolbar-items>
        <v-btn dark text @click="save" :disabled="!valid"> Lưu</v-btn>
      </v-toolbar-items>
      <v-spacer></v-spacer>

      <v-toolbar-title>Thay đổi chương trình</v-toolbar-title>
      <v-spacer></v-spacer>
...
export default {
  ...
  data() {
    return {
      ...
      valid: true,
      fromDateToDate: [],
      applyOptions: ['all', 'service_categories', 'services', 'variants'],
      fromDateMenu: false,
      toDateMenu: false,
  ...
  methods: {
    ...mapActions('discount', ['updateDiscount']),
    applyToHandle(val) {
      if (val === 'member') {
    ...
    open() {
      this.dialog = true;
      this.discountForm = JSON.parse(JSON.stringify(this.discount));
      this.fromTime = this.discountForm.fromTime.split(':');
      this.toTime = this.discountForm.toTime.split(':');
      this.fromDateToDate = [
        this.discountForm.fromDate,
        this.discountForm.toDate,
      ];
    },
    cancel() {
      this.discountForm = {
    ...
    async save() {
      ...
      try {
        await this.updateDiscount({ ...this.discountForm });
